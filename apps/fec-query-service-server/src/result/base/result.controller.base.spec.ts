...
const service = {
  ...
  results: () => FIND_MANY_RESULT,
  result: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Result", () => {
  ...
  test("GET /results/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/results"}/${nonExistingId}`)
  ...
  test("GET /results/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/results"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /results existing resource", async () => {
